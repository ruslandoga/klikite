...
config :k, K.Repo,
  database: Path.expand("../k_dev.db", Path.dirname(__ENV__.file)),
  pool_size: 5,
  stacktrace: true,
...
config :k, KWeb.Endpoint,
  ...
  watchers: [
    ...
    npm: ["run", "watch:tracker", cd: Path.expand("../assets", __DIR__)],
...
config :k, KWeb.Endpoint,
  live_reload: [
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/k_web/(live|views)/.*(ex)$",
      ~r"lib/k_web/templates/.*(eex)$"
    ]
  ]
